...
@Component({
	selector: 'ngx-hotSearchListTemplate',
	templateUrl: './hotSearchListTemplate.component.html',
	styleUrls: ['./hotSearchListTemplate.component.scss']
})
export class HotSearchListTemplateComponent implements OnInit,OnChanges {
	@Input() listItem: any;
	...
	imgonload(e:any){
		this.count+=1;
		if(this.count==this.listItem.infoArr.length){
			console.log('全部加载完成')
			this.imgShow = true;
		}
	}
	...
	doPlay(cIndex : any) {
		this.change.emit({cIndex,i:this.listIndex});
	}
}