...
export class MultiModalLearningComponent implements OnInit {
	...
	play(element : any) {
		this.pause();
		var i = this.qqkgData.qqData.findIndex((e : any) => e.musicUrl == element.srcElement.currentSrc)
		if (i !== -1) {
			this.qqkgData.qqData[i].isPlay = true;
		}
		var i2 = this.qqkgData.kgData.findIndex((e : any) => e.musicUrl == element.srcElement.currentSrc)
		if (i2 !== -1) {
	...
	ObservationalData() {
		...
		let KQbjId = [{ MixSongID: this.chooseQQKGData.kgData.MixSongID, FileHash: this.chooseQQKGData.kgData.FileHash }]
		let obj = {
				QMid, Qid, KScid, KQbjId
			}
		return new Promise((resolve : any) => {
			this.api.ObservationalData(obj).subscribe((res : any) => {
				if (res.success) {
					res.data.KG[0].KExponents = res.data.KG[0].KExponents || {data:{}}
	...
	TJMusicMultiModalLearning() {
		...
		return new Promise((resolve : any) => {
			this.api.TJMusicMultiModalLearning(data).subscribe((res : any) => {
				if (res.success) {
	...
	ObservationDataStorage(MeasuredParameter:any,PlatformParameter:any) {
		...
		if(this.chooseQQKGData.kgData && this.chooseQQKGData.qqData && !(this.obData.QQ && this.obData.KG)){
			console.log("获取双平台数据错误 不存入")
			return;
		}
		this.api.ObservationDataStorage({
			"UserId": this.userId,
			"KeyWord":this.keyword,
