...
export class DouyinVideoComponent implements OnInit {
		...
		this.api
			...
			.subscribe(
				async (res: any) => {
					...
					// 获取视频详情 获取热点id
					await this.getDouIndexOfSentenceId(res.result)
	...
	douyin_videoisListen() {
		...
		this.api
			.douyin_videoisListen({
				secUid: this.BloggerInfo.secUid,
		...
		if (result.textExtra.length > 0) {
			for (let i = 0; i < result.textExtra.length; i++) {
				...
				if (i == result.textExtra.length - 1) {
					textExtra += result.textExtra[i].hashtag_name
						? result.textExtra[i].hashtag_name
						: "";
				} else {
					textExtra += result.textExtra[i].hashtag_name
						? result.textExtra[i].hashtag_name + ","
		...
		return new Promise((resolve: any) => {
			this.api
				.douyin_listenVideo({
					...
					commentText,
					watchComment: this.watchComment ? 1 : 0,
					secUid: this.BloggerInfo.secUid,
					commentSecUid: this.commentSecUid,
					awemeId: this.awemeId,
					title: result.title,
					admireCount: result.admireCount,
					collectCount: result.collectCount,
					commentCount: result.commentCount,
					diggCount: result.diggCount,
					playCount: result.playCount,
					previewTitle: preTitle,
	...
	listeChange() {
		...
		if (
			this.listeValue.indexOf("https://v.douyin.com/") != -1 ||
			this.listeValue.indexOf("modal_id=") != -1 ||
			!isNaN(Number(this.listeValue))
		) {
		...
		//判断不符合规则的 不查询
		var isSearch = valueArr.every(
			(e: any) =>
				(e + "").indexOf("https://v.douyin.com/") != -1 ||
				(e + "").indexOf("modal_id=") != -1 ||
				((e + "").indexOf(".") == -1 &&
					!isNaN(Number(e)) &&
					(e + "").length > 11)
		);
	...
	searchVideoList(valueArr: any) {
		...
		this.loading = true;
		this.api.DouYinSearchVideoDetailsList({ type, arr: valueArr }).subscribe(
			(res: any) => {
				console.log(res);
				this.loading = false;
				if (res.success) {
	...
	resultList: any = [];
	// 作者趋势
	toAuthorDetail(item: any) {
		if (item.seeVideo3) {
			item.isShowRadio3 = item.isShowRadio3 ? false : true;
			item.isShowRadio = false;
			item.isShowRadio2 = false;
		} else {
			this.douAuthorDetail(item);
		}
	}
	douAuthorDetail(data: any) {
		this.loading = true;
		data.seeVideo3 = true;
		this.api
			.getDouAuthorInfo({ sec_uid: data.BloggerInfo.secUid })
			.subscribe((res: any) => {
				this.loading = false;
				if (res.success) {
