...
@Component({
	selector: 'ngx-douyin-listenDarenYXH',
	templateUrl: './douyin-listenDaren.component.html',
	styleUrls: ['./douyin-listenDaren.component.scss']
})
export class DouyinListenDarenYXHComponent implements OnInit {
	...
	douyin_darenTypeList(){
		  this.api.douyin_darenTypeListMarketability().subscribe((res: any) => {
	      if (res.success) {
	...
	DouYinSearchVideoDetailsList(aweme_idArr:any){
		return new Promise((resolve)=>{
			this.api.DouYinSearchVideoDetailsListMarketability({arr:aweme_idArr,type:'ID'})
			.subscribe((res: any) => {
					if (res.success) {
						resolve(res.result)
					}else{
	...
	douyin_getListenDaren() {
		this.loading = true;
		this.api.douyin_getListenDarenMarketability({searchValueNote:this.searchValueNote,yirenType:this.yirenType ,feesMax: this.feesMax, feesMin: this.feesMin, minimumMax: this.minimumMax, minimumMin: this.minimumMin, diggCountAveMax: this.diggCountAveMax, diggCountAveMin: this.diggCountAveMin, activityNum: this.activityNum, pageSize: this.pageSize, page: this.page, keyword: this.searchValue, type: this.type ? '我的' : '全部' })
			.subscribe((res: any) => {
				console.log(res)
				if (res.success) {
	...
	// 点击录入
	handleOk(): void {
		let { dy_Monitoring_ID, dy_BloggerInfo_ID, Information,typeList, Fees, Note, SecUid, FeesShow } = this.editItem;
		...
		// }
		this.loading = true;
		this.api.douyin_listenDarenEditMarketability({ dy_Monitoring_ID, dy_BloggerInfo_ID, Information,TypeJson, Fees, Note, SecUid, FeesShow }).subscribe((res: any) => {
			console.log(res)
			this.loading = false;
			if (res.success) {
