...
@Component({
  selector: "ngx-douyin-darenYXH",
  templateUrl: "./douyin-daren.component.html",
  styleUrls: ["./douyin-daren.component.scss"],
})
export class DouyinDarenYXHComponent implements OnInit {
  constructor(
  ...
  DouYinSearchVideoDetails() {
  	...
  	this.loading = true;
  	this.api
  		.DouYinSearchBigVForVideoLinkMarketability({ link: v})
  		.subscribe(
  			async (res: any) => {
  				console.log(res);
  				if(res.success){
  ...
  DouYinSearchVideoDetailsList(aweme_idArr: any) {
    return new Promise((resolve) => {
      this.api
        .DouYinSearchVideoDetailsListMarketability({ arr: aweme_idArr, type: "ID" })
        .subscribe(
          (res: any) => {
            if (res.success) {
              resolve(res.result);
  ...
  douyin_isListen(item: any) {
    this.loading = true;
    this.api.douyin_isListenMarketability({ SecUid: item.secUid }).subscribe(
      (res: any) => {
        console.log(res);
        this.loading = false;
  ...
  douyin_listenDaren() {
    ...
    this.loading = true;
    this.api
      .douyin_listenDarenMarketability({
        feesShow,
        signature,
        urlList,
        nickname,
...
paginationShow=true
  ...
  douyin_darenSearch() {
    this.loading = true;
    this.api
      .DouYinSearchBigVMarketability({ page: this.page, keyword: this.searchValue })
      .subscribe(
        (res: any) => {
          console.log(res);
          this.loading = false;
